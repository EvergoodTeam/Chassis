...
public class ConfigSerializer {
    ...
    // TODO: can be optimized, haven't checked with multiple resource locks
    public List<String> getWrittenUserSection() {
        List<String> result = new ArrayList<>();
        Optional<List<String>> contents = getWrittenContents();

        contents.ifPresent(strings -> {
            int starterLine = 0;
            // This assumes the resourceLock cat is the furthest down from the section generated by default
            List<String> currentLock = resourceLock();
            String lastLockOption = StringUtils.firstFromSplit(currentLock.get(currentLock.size() - 1), " = ");

            for (String line : strings) {
                // Account for option having a different value: this will assume last line will be an option and not a comment
                String actual = StringUtils.firstFromSplit(line, " = ");
                if (actual.equals(lastLockOption)) {
                    starterLine = strings.indexOf(line) + 1;
                    break;
                }
            }

            for (int i = starterLine; i < strings.size(); i++) {
